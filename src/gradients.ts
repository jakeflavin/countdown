/**
 * The gradient backgrounds, offered by the custom theme. These are the six palettes the
...
export type Gradient = {
  ...
  name: string
  background: string
  /** The flat colour under the gradient, painted before it and behind any letterbox. */
  base: string
  text: string
  muted: string
  surface: string
...
export function gradientById(id: string): Gradient {
  return gradients.find((g) => g.id === id) ?? defaultGradient
}
